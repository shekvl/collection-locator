...
const definition = reactive({ name: '', targetK: 0, fast: false, batch: false, columns: [] });
const loaded = ref(false);
const saved = ref(false);
var codelabel = "";

onMounted(async () => {
    const resp = await getDefinitionForId(route.params.id);
    if (resp.success) {
        // add id property to each column: codeText(code)
        definition.name = resp.data.name;
        definition.fast = resp.data.fast;
        definition.batch = resp.data.batch;
        definition.targetK = resp.data.targetK;
        definition.columns = resp.data.columns.map((col) => ({
            name: col.name,
            cdm: { id: `${col.codeText} (${col.code})`, text: `${col.codeText} (${col.code})`, slug: col.codeText},
        }));
        loaded.value = true;
        saved.value = false;
    } else {
        console.error('Error loading definition:', resp.message);
    }
});
...
async function save() {
    await updateDefinitionDirect(route.params.id, definition);
    saved.value = true;
} </script>
<style>
form {
